...
class SoccerEmptyGoalEnv(SoccerEnv):
    ...
    def __init__(self):
        ...
        self.old_ball_velocity = 0
    def new_get_reward(self):
        current_state = self.env.getState()
        ball_proximity = current_state[53]
        ball_velocity = current_state[55]
        reward=0
        if ball_proximity > self.old_ball_prox:
            reward += 0.1
        if ball_velocity > self.old_ball_velocity and current_state[57] > 0:
            reward += 0.2
        ...
        if self.status == hfo_py.OUT_OF_BOUNDS:
            reward -= 5
        self.old_ball_velocity = current_state[55]
    ...
    def _get_reward(self):
        ...
        and scoring a goal.
        """
        # CHANGED REWARD TO GIVE NEGATIVE REWARD FOR GOING OUTSIDE THE PLAYABLE AREA.
        # reward function had a bug which gives positive reward for going outside the playable area.
        current_state = self.env.getState()
        ball_proximity = current_state[53]
...
#            reward += (1 - current_state[51]) * 0.001 * (int(current_state[52] > 0) * 2 - 1) 
            ...
            if current_state[46]**2 < 1 and current_state[48]**2 < 1:
                self.out_of_bounds_count = 0
                reward += 0.6 * -ball_dist_goal_delta
            else:
                self.out_of_bounds_count = 1
                reward -= 0.1 * self.out_of_bounds_count
