...
class ActorNetwork(object):
    ...
    def __init__(self, sess, state_dim, action_dim, action_bound_space, action_bound_low, learning_rate, tau):
        ...
        decay_steps = int(start_decay_step / 10)
        decay_factor = 0.5
        ...
        self.optimize = tf.train.AdamOptimizer(self.learning_rate).\
            apply_gradients(zip(self.actor_gradients, self.network_params), global_step=self.global_step)
    ...
    def create_actor_network(self):
        ...
        scaled_out = tf.add(tf.multiply(out, self.action_bound_space), self.action_bound_low)
...
class CriticNetwork(object):
    ...
    def create_critic_network(self):
        ...
        net = tflearn.activations.leaky_relu(tflearn.fully_connected(tf.concat((input_net, action_net), 1), 300), alpha=0.01)
        net = tflearn.activations.leaky_relu(tflearn.fully_connected(net, 200), alpha=0.01)
    ...
    def __init__(self, sess, state_dim, action_dim, learning_rate, tau, num_actor_vars):
        ...
        self.grads_norm = tf.norm(self.action_grads)
...
def build_summaries(actor, critic):
    ...
    tf.summary.scalar("Number of steps per episode", step_counter)

    summary_vars = [episode_reward, episode_ave_max_q, episode_loss, grads_norm, episode_exe_time, step_counter]
